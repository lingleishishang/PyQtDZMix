...
from Ui_PyOriginalData import Ui_MainWindow
import numpy as np
...
import SircombeHazeltonDistance as sh
import KUTest as kut
import KSTest as ks
import WassersteinDistance as wd
import QQ as Qr
import csv
from scipy.interpolate import griddata
from mpl_toolkits.mplot3d import Axes3D  # 关键：启用3D投影功能
from scipy.spatial import ConvexHull
        # 2. 自定义表格模型（继承 QAbstractTableModel）
...
class PyOriData(QMainWindow, Ui_MainWindow):
    """
...
#        plt.figure()
#        for a in range(self.pair_count):
#            plt.plot(self.T, self.np_source[a], color=self.color[a], label=self.headers[a*2 + 2])  
#        
#        plt.plot(self.T, self.np_sample, color='black', label=self.headers[0])
#        plt.ylim(bottom=0)
#        plt.xlim(self.age_range[0], self.age_range[1])
#        plt.legend()
#        plt.show()
    def __init__(self, parent=None):
        ...
        @type QWidget
        """
        super(PyOriData, self).__init__(parent)
        self.setupUi(self)
...
#        self.groupBox_7.setGeometry(width/4, 2*height/3, width/4, height/3)
        ...
        self.lineEdit.setGeometry(0,self.groupBox_3.height()/3,self.groupBox_3.width()/4,30)
        self.lineEdit_2.setGeometry(self.groupBox_3.width()/3,self.groupBox_3.height()/3,self.groupBox_3.width()/4,30)
        self.lineEdit_8.setGeometry(self.groupBox_3.width()*2/3,self.groupBox_3.height()/3,self.groupBox_3.width()/4,30)        
        
        
        self.groupBox.setGeometry(
            int(0),          # x坐标（左边界）
            int(height/7+height/25),       # y坐标（上边界）
            int(width/4-width/100),     # 宽度（按原比例）
            int(height/7)          # 高度（按原比例）
        )
...
#                self.plot_results()
                ...
                #self.canvas.figure.set_size_inches(0.8*self.groupBox_2.width(), 0.8*self.groupBox_2.width()-900)  # 宽10英寸，高4英寸（高度降低，避免文字被压缩）
...
#        if number >3 and number <=10:
#            print('yes')
#            self.age_range=[int(self.lineEdit.text()),int(self.lineEdit_2.text())]
#            self.dT=int(self.lineEdit_8.text())
#            self.T=np.zeros((self.age_range[1]-self.age_range[0])//self.dT)
#            if self.radioButton.isChecked():
#            
#                for i in range(len(self.T)):
#                    self.T[i]=self.age_range[0]+i * self.dT
#                self.pair_count = (len(self.headers) - 2) // 2
#                self.source = [[] for _ in range(pair_count)]   # [[], [], ...]
#                self.se     = [[] for _ in range(pair_count)]
#                self.np_source = [[] for _ in range(pair_count)]
#                for j in range(self.pair_count):
#                    self.source[j]=self.data[j*2+2]
#                    self.se[j]=self.data[j*2+3]
#                    self.np_source[j]=np.zeros(len(self.T))
...
#                    self.np_source[a] = self.np_source[a] /len(self.source[a])
#                    plt.plot(self.T,self.np_source[a],color=self.color[a],label=self.headers[a*2+2])  
#                self.np_sample=self.np_sample/len(self.np_sample)
#                plt.plot(self.T,self.np_sample,color='black',label=self.headers[0])
#                plt.ylim(bottom=0)
#                plt.xlim(int(self.lineEdit.text()),int(self.lineEdit_2.text()))
#                plt.legend()
#                plt.show()
#            if self.radioButton_2.isChecked():
#                pass
...
#                    
    ...
    def on_pushButton_4_clicked(self):
        ...
        Slot documentation goes here.
        """
        # 1. 清空画布（关键：使用 canvas6 绑定的 figure）
        ...
        if num == 2 and self.radioButton.isChecked():
            self.aa = []
            self.value1=[]
            self.canvas2.figure.clear()
            self.process_data(min_age, max_age, dT_value)
            Ratio_A = [i/100 for i in range(101)]
            self.aa = [
                ks.kstest2b_tnc(self.np_sample, (i/100)*self.np_source[0] + ((100-i)/100)*self.np_source[1],0.05,'unequal',len(self.T))[2]
                for i in range(101)
            ]
            self.max_aa=np.min(self.aa)
            self.max_aa_index=np.argmin(self.aa)
            ...
            if len(x_unique) >= 2:
                ...
                interp_func = interp1d(x_unique, z_unique, kind="linear", bounds_error=False, fill_value=np.min(z_data))
...
#                fig.clear()  # 清除原有图形
                ...
                # 第一个子图（占据2/3宽度）
                ax1 = self.canvas.figure.add_subplot(gs[0])
                self.all=self.max_Ratio_A*self.np_source[0]+(1-self.max_Ratio_A)*self.np_source[1]
...
#        
        ...
        if num == 3 and self.radioButton.isChecked():
                    # ---------------------- 初始化空列表（放在循环外！） ----------------------
            ...
            for i in range(101):     
                for j in range(101 - i):
                    ...
                    self.aa.append(ks.kstest2b_tnc(self.np_sample, 
                                (i/100)*self.np_source[0] + 
                                (j/100)*self.np_source[1] + 
                                ((100-i-j)/100)*self.np_source[2],0.05,'unequal',len(self.T)
                            )[2])
            ...
            self.value1.append(min(self.aa))            
            ...
            ax.pcolormesh(grid_x, grid_y, grid_z, cmap='viridis', alpha=0.8, shading='gouraud')
            ...
            # 3. 绘制原始数据点（可选）
            ax.scatter(x, y, c=values, cmap='viridis', s=5, alpha=0.3)
            ...
            # 第一个子图（占据2/3宽度）
            ax1 = self.canvas.figure.add_subplot(gs[0])
            self.all=A_max*self.np_source[0]+B_max*self.np_source[1]+C_max*self.np_source[2]
            ax1.plot(self.T, self.all, color='blue', label='option model')  
...
#        
        ...
        if num == 4 and self.radioButton.isChecked():
                ...
                for j in range(0,101 - i,3):
                    for k in range(101-i-j):
                        ...
                        self.aa.append(ks.kstest2b_tnc(self.np_sample, 
                                    (i/100)*self.np_source[0] + 
                                    (j/100)*self.np_source[1] + 
                                    (k/100)*self.np_source[2]+((100-i-j-k)/100)*self.np_source[3],0.05,'unequal',len(self.T)
                                )[2])
            ...
            # 绘制散点图
            ax1.scatter(x, y, z, c=values, cmap='viridis', s=5, alpha=0.3)
...
#                    ax.text(*label_pos, f"{fraction:.1f}", fontsize=8, ha='center', va='center')
            ...
            self.all=A_max*self.np_source[0]+B_max*self.np_source[1]+C_max*self.np_source[2]+D_max*self.np_source[3]
            ax1.plot(self.T, self.all, color='blue', label='option model')  
...
#        
            ...
            self.canvas.draw()   
        if self.radioButton_2.isChecked():
            #self.lineEdit_3.setEnabled(True)  # 设为可编辑
            if num == 2 :
    ...
    def on_pushButton_10_clicked(self):
        ...
        if num == 2 and self.radioButton.isChecked():
            ...
            Ratio_A = [i/100 for i in range(101)]
            self.aa = [
                kut.kuipertest2_tnc(self.np_sample, (i/100)*self.np_source[0] + ((100-i)/100)*self.np_source[1],len(self.T))[1]
                for i in range(101)
            ]
            self.max_aa=np.min(self.aa)
            self.max_aa_index=np.argmin(self.aa)
...
#        
        ...
        if num == 3 and self.radioButton.isChecked():
            ...
            for i in range(101):     
                for j in range(101 - i):
                    ...
                    self.aa.append(kut.kuipertest2_tnc(self.np_sample, 
                                (i/100)*self.np_source[0] + 
                                (j/100)*self.np_source[1] + 
                                ((100-i-j)/100)*self.np_source[2],len(self.T)
                            )[1])
            ...
            self.value2.append(min(self.aa))  
            print("最高相似度点比例：")
            print(f"A={A_max:.3f}, B={B_max:.3f}, C={C_max:.3f}")
            print(f"最高相似度值：{values[idx_max]:.4f}")
            # ---------------------- 在 canvas6 中绘制三元相图 ----------------------
                ...
                for j in range(0,101 - i,3):
                    for k in range(101-i-j):
                        ...
                        self.aa.append(kut.kuipertest2_tnc(self.np_sample, 
                                    (i/100)*self.np_source[0] + 
                                    (j/100)*self.np_source[1] + 
                                    (k/100)*self.np_source[2]+((100-i-j-k)/100)*self.np_source[3],len(self.T)
                                )[1])
...
#        
            ...
            self.canvas.figure.set_constrained_layout(True)
                ...
                #plt.subplots_adjust(left=0.08, right=0.95, top=0.92, bottom=0.2)  # 手动调整边距
            self.canvas.draw()   
        if num == 2 and self.radioButton_2.isChecked():
            self.aa = []
            self.value2=[]
            self.canvas3.figure.clear()
            self.process_data_kde(min_age, max_age, dT_value,int(self.lineEdit_3.text()))
...
#        
        ...
        if num == 3 and self.radioButton_2.isChecked():
                    # ---------------------- 初始化空列表（放在循环外！） ----------------------
            self.ratios = []  # 存储所有 [A, B, C] 比例
            self.aa = []      # 存储所有相似度值
            self.value2=[]
...
#        
        ...
        if num == 4 and self.radioButton_2.isChecked():
...
#        
    ...
    def on_pushButton_13_clicked(self):
        ...
        if num == 2 and self.radioButton.isChecked():
            ...
            Ratio_A = [i/100 for i in range(101)]
            self.aa = [
                wd.calculate_wasserstein_2(self.np_sample, (i/100)*self.np_source[0] + ((100-i)/100)*self.np_source[1])[0]
                for i in range(101)
            ]
            self.slope = [
                wd.calculate_wasserstein_2(self.np_sample, (i/100)*self.np_source[0] + ((100-i)/100)*self.np_source[1])[1]
                for i in range(101)
            ]
            self.intercept = [
                wd.calculate_wasserstein_2(self.np_sample, (i/100)*self.np_source[0] + ((100-i)/100)*self.np_source[1])[2]
                for i in range(101)
            ]
            self.max_aa=np.min(self.aa)
            self.max_aa_index=np.argmin(self.aa)
            ...
            print(self.aa)
            print(self.max_Ratio_A,1-self.max_Ratio_A,self.max_aa)
            self.value3.append('Wasserstein-2')
            ...
            if len(x_unique) >= 2:
                ...
                cbar.set_label("Wasserstein-2", rotation=0, labelpad=5)
...
#
#                ax3.grid(alpha=0.3)
                ax3.text(max(self.np_sample)/2, 4*max(self.all)/5, f'Value= {self.max_aa:.2f}', fontsize=12,
        ...
        if num == 3 and self.radioButton.isChecked():
            ...
            for i in range(101):     
                for j in range(101 - i):
                    self.ratios.append([i/100, j/100, (100 - i - j)/100])  # A, B, C 比例
                    self.aa.append(wd.calculate_wasserstein_2(self.np_sample, 
                                (i/100)*self.np_source[0] + 
                                (j/100)*self.np_source[1] + 
                                ((100-i-j)/100)*self.np_source[2]
                            )[0])
                    self.slope.append(wd.calculate_wasserstein_2(self.np_sample, 
                                (i/100)*self.np_source[0] + 
                                (j/100)*self.np_source[1] + 
                                ((100-i-j)/100)*self.np_source[2]
                            )[1])
                    self.intercept.append(wd.calculate_wasserstein_2(self.np_sample, 
                                (i/100)*self.np_source[0] + 
                                (j/100)*self.np_source[1] + 
                                ((100-i-j)/100)*self.np_source[2]
                            )[2])                    
...
#            print(self.bb)
            ...
            self.value3.append(min(self.aa))
...
#            ax3.set_aspect("equal") 
            #ax3.grid(alpha=0.3)
            ax3.text(max(self.np_sample)/2, 4*max(self.all)/5, f'Value= {max(values):.2f}', fontsize=12,
                     bbox=dict(facecolor='white', alpha=0.8))
            #ax3.legend(loc='upper left')
                ...
                for j in range(0,101 - i,3):
                    for k in range(101-i-j):
                        ...
                        self.aa.append(wd.calculate_wasserstein_2(self.np_sample, 
                                    (i/100)*self.np_source[0] + 
                                    (j/100)*self.np_source[1] + 
                                    (k/100)*self.np_source[2]+((100-i-j-k)/100)*self.np_source[3]
                                )[0])
                        self.slope.append(wd.calculate_wasserstein_2(self.np_sample, 
                                    (i/100)*self.np_source[0] + 
                                    (j/100)*self.np_source[1] + 
...
#                    ax.text(*label_pos, f"{fraction:.1f}", fontsize=8, ha='center', va='center')
            ...
            sm = plt.cm.ScalarMappable(cmap='viridis', norm=plt.Normalize(0, 1))
            sm.set_array([])
            cbar = self.canvas4.figure.colorbar(
                sm, 
                ax=ax1, 
                pad=0.05,          # 与图形的间距
                aspect=40,         # 核心：减小aspect值（默认20，值越小颜色条越“短粗”）
            ...
            cbar.set_label("Wasserstein-2 distance", rotation=0, labelpad=5)  # 标签水平放置，更易读
...
#        
    ...
    def on_pushButton_16_clicked(self):
        ...
        if num == 2 and self.radioButton.isChecked():
            ...
            Ratio_A = [i/100 for i in range(101)]
            self.aa = [
                sh.SircombeHazeltonDistance(self.np_sample, (i/100)*self.np_source[0] + ((100-i)/100)*self.np_source[1])
                for i in range(101)
            ]
            self.max_aa=np.min(self.aa)
            self.max_aa_index=np.argmin(self.aa)
            ...
            print(self.aa)
            print(self.max_Ratio_A,1-self.max_Ratio_A,self.max_aa)
            self.value6.append('Sircombe-Hazelton')
            ...
            if len(x_unique) >= 2:
                ...
                cbar.set_label('Sircombe-Hazelton', rotation=0, labelpad=5)
...
#        
        ...
        if num == 3 and self.radioButton.isChecked():
            ...
            for i in range(101):     
                for j in range(101 - i):
                    self.ratios.append([i/100, j/100, (100 - i - j)/100])  # A, B, C 比例
                    self.aa.append(sh.SircombeHazeltonDistance(self.np_sample, 
                                (i/100)*self.np_source[0] + 
                                (j/100)*self.np_source[1] + 
                                ((100-i-j)/100)*self.np_source[2]
                            ))
            ...
            self.value6.append(min(self.aa))
            ...
            cbar.set_label("Sircombe-Hazelton", rotation=0, labelpad=10)
...
#                    ax.text(*label_pos, f"{fraction:.1f}", fontsize=8, ha='center', va='center')
            ...
            sm = plt.cm.ScalarMappable(cmap='viridis', norm=plt.Normalize(0, 1))
            sm.set_array([])
            cbar = self.canvas7.figure.colorbar(
                sm, 
                ax=ax1, 
                pad=0.05,          # 与图形的间距
                aspect=40,         # 核心：减小aspect值（默认20，值越小颜色条越“短粗”）
            ...
            cbar.set_label("Sircombe-Hazelton", rotation=0, labelpad=5)  # 标签水平放置，更易读
...
#        
    ...
    def on_pushButton_19_clicked(self):
        ...
        if num == 2 and self.radioButton.isChecked():
            ...
            Ratio_A = [i/100 for i in range(101)]
            self.q1=[
                Qr.calculate_qq_plot(self.np_sample, (i/100)*self.np_source[0] + ((100-i)/100)*self.np_source[1],500)[0]
                for i in range(101)
            ]
            self.q2=[
                Qr.calculate_qq_plot(self.np_sample, (i/100)*self.np_source[0] + ((100-i)/100)*self.np_source[1],500)[1]
                for i in range(101)
            ]
            self.aa = [
                Qr.calculate_qq_plot(self.np_sample, (i/100)*self.np_source[0] + ((100-i)/100)*self.np_source[1],500)[2]
                for i in range(101)
            ]
            self.slope = [
                Qr.calculate_qq_plot(self.np_sample, (i/100)*self.np_source[0] + ((100-i)/100)*self.np_source[1],500)[3]
                for i in range(101)
            ]
            self.intercept = [
                Qr.calculate_qq_plot(self.np_sample, (i/100)*self.np_source[0] + ((100-i)/100)*self.np_source[1],500)[4]
                for i in range(101)
            ]
...
#        
                ...
                ax3.set_aspect("equal")  # 关键参数
                ax3.text(max_val/2, 0.9*max_val, f'Value = {max(self.aa):.2f}', fontsize=12,
                                bbox=dict(facecolor='white', alpha=0.8))
            ...
            for i in range(101):     
                for j in range(101 - i):
                    ...
                    self.q1.append(Qr.calculate_qq_plot(self.np_sample, 
                                (i/100)*self.np_source[0] + 
                                (j/100)*self.np_source[1] + 
                                ((100-i-j)/100)*self.np_source[2],500
                            )[0])
                ...
                for j in range(0,101 - i,3):
                    for k in range(101-i-j):
                        ...
                        self.q1.append(Qr.calculate_qq_plot(self.np_sample, 
                                    (i/100)*self.np_source[0] + 
                                    (j/100)*self.np_source[1] + 
                                    (k/100)*self.np_source[2]+((100-i-j-k)/100)*self.np_source[3],500
                                )[0])
                        self.q2.append(Qr.calculate_qq_plot(self.np_sample, 
                                    (i/100)*self.np_source[0] + 
                                    (j/100)*self.np_source[1] + 
...
#                    ax.text(*label_pos, f"{fraction:.1f}", fontsize=8, ha='center', va='center')
            ...
            sm = plt.cm.ScalarMappable(cmap='viridis', norm=plt.Normalize(0, 1))
            sm.set_array([])
            cbar = self.canvas5.figure.colorbar(
                sm, 
                ax=ax1, 
                pad=0.05,          # 与图形的间距
                aspect=40,         # 核心：减小aspect值（默认20，值越小颜色条越“短粗”）
            ...
            cbar.set_label("Value", rotation=0, labelpad=5)  # 标签水平放置，更易读
...
#        
    ...
    def on_pushButton_22_clicked(self):
        ...
        if num == 2 and self.radioButton.isChecked():
            ...
            Ratio_A = [i/100 for i in range(101)]
            self.aa = [
                sim.Similarity(self.np_sample, (i/100)*self.np_source[0] + ((100-i)/100)*self.np_source[1])
                for i in range(101)
            ]
            self.max_aa=np.max(self.aa)
            self.max_aa_index=np.argmax(self.aa)
...
#        
        ...
        if num == 3 and self.radioButton.isChecked():
            ...
            sm = plt.cm.ScalarMappable(cmap='viridis', norm=plt.Normalize(0, 1))
            sm.set_array([])
            cbar = self.canvas6.figure.colorbar(sm, ax=ax, pad=0.05, orientation="horizontal", aspect=40)
...
#            [0.7]                  # 第3行数据（尺寸=1）
#        ]
#        method=['K-S','Kupier','Wasserstein-2','Q-Q','Simlarity','Sircombe-Hazelton']
#        
