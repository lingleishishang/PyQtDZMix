...
#        print(self.sample)
        ...
        if self.radioButton_2.isChecked() and self.radioButton_3.isChecked():
            ...
            for i, combo in enumerate(random_combinations, 1):
                ...
                self.r2.append(wd.calculate_wasserstein_2(self.np_sample, self.sum)[0])
...
#            self.qinterceptr=Qr.calculate_qq_plot(self.np_sample, self.np_source,len(self.T))[4]     
                ...
                self.like.append(sh.SircombeHazeltonDistance(self.np_sample,self.sum))
...
#            print(self.aa)
            idx_max1 = np.argmin(self.ks)
            print(self.ratios[idx_max1])
            print(min(self.ks))
            idx_max2 = np.argmin(self.kut)
            print(self.ratios[idx_max2])
            print(min(self.kut))
            idx_max3 = np.argmin(self.r2)
...
#        
            ...
            ax3.plot(self.T, self.sum3, color=self.generate_100_colors()[2], label='Wasserstein-2')
            self.sum4=0
            ...
            ax3.plot(self.T, self.sum6, color=self.generate_100_colors()[5], label='Sircombe-Hazelton')            
            ...
            ax4.plot(self.T, np.cumsum(self.sum3), color=self.generate_100_colors()[2], label='Wasserstein-2')
            ...
            ax4.plot(self.T, np.cumsum(self.sum6), color=self.generate_100_colors()[5], label='Sircombe-Hazelton')
            ...
            # 4. 设置行名（垂直表头）
            row_names = ['K-S','Kupier','Wasserstein-2','Q-Q','Simlarity','Sircombe-Hazelton']
...
#            
            ...
            item1 = QStandardItem(str("{:.4f}".format(min(self.ks)))) 
            ...
            item2 = QStandardItem(str("{:.4f}".format(min(self.kut)))) 
...
#                    
                    ...
                    self.kutmax.append(min(self.kut))
                    ...
                    self.likemax.append(min(self.like))  
                print('*'*100)        
                print('final')
                print(self.aaa)
                k1=np.argmin(self.ksmax)
                print(self.ksmaxr[k1])
                print(min(self.ksmax))   
                
                k2=np.argmin(self.kutmax)
                print(self.kutmaxr[k2])
                print(min(self.kutmax))  
                
                k3=np.argmin(self.r2max)
                print(self.r2maxr[k3])
                print(min(self.r2max))   
                ...
                k6=np.argmin(self.likemax)
                print(self.likemaxr[k6])
                print(min(self.likemax))  
#            
#            
#            
...
#            
            for i in range(len(self.ksmaxr[k1])):
                ...
                model.setItem(0, i, item)
            item1 = QStandardItem(str("{:.4f}".format(min(self.ksmax)))) 
            model.setItem(0, len(self.ksmaxr[k1]), item1)
            
            for i in range(len(self.kutmaxr[k2])):
                item = QStandardItem(str("{:.4f}".format(self.kutmaxr[k2][i]/100)))  
                model.setItem(1, i, item)
            item2 = QStandardItem(str("{:.4f}".format(min(self.kutmax)))) 
...
#                    min_length = min(len(lst) for lst in self.dd)
                ...
                #self.aa.append(sim.Similarity(self.np_sample, self.sum))
                        ...
                        self.r2.append(wd.calculate_wasserstein_2(np_sample, self.sum)[0])                      
                        self.qr2.append(Qr.calculate_qq_plot(np_sample, self.sum,len(self.T))[2])
                        self.sim.append(sim.Similarity(np_sample,self.sum))
                        self.like.append(sh.SircombeHazeltonDistance(np_sample,self.sum))
                    print(str(j+1)+'次')
