...
#        print(self.source)
        if self.radioButton_2.isChecked() and self.radioButton_3.isChecked():
            ...
            self.r2=wd.calculate_wasserstein_2(self.np_sample, self.np_source)[0]
            self.slopr2=wd.calculate_wasserstein_2(self.np_sample, self.np_source)[1]
            self.interceptr2=wd.calculate_wasserstein_2(self.np_sample, self.np_source)[2]
            ...
            self.like=sh.SircombeHazeltonDistance(self.np_sample,self.np_source)
...
#        
            ...
            #ax3.grid(alpha=0.3)
            ax3.text(np.max(self.np_sample)/2, 4*max(np.max(self.np_sample), np.max(self.np_source))/5, f'Value = {self.r2:.2f}', fontsize=12,
            ...
            ax4.text(max_val/2, 0.9*max_val, f'Value = {self.qr2:.2f}', fontsize=12,
                            bbox=dict(facecolor='white', alpha=0.8))
            # 调整子图间距
        ...
        if self.radioButton_3.isChecked() and self.radioButton.isChecked():
            ...
            if min_length < int(self.lineEdit_4.text()):
                ...
                for i in range(len(all_sources)):
                    ...
                    self.r2.append(wd.calculate_wasserstein_2(all_samples[-1], all_sources[i])[0])
#                    self.slopr2=wd.calculate_wasserstein_2(all_samples[-1], all_sources[i])[1]
#                    self.interceptr2=wd.calculate_wasserstein_2(all_samples[-1], all_sources[i])[2]
#                    
...
#                    self.qinterceptr=Qr.calculate_qq_plot(all_samples[-1], all_sources[i],len(self.T))[4]     
                    ...
                    self.like.append(sh.SircombeHazeltonDistance(all_samples[-1], all_sources[i]))                    
