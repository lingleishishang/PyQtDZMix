...
#            print(self.aaa[0])
            for j in range (len(self.aaa6)):
                for i in range(101):
                    ...
                    ksks=sh.SircombeHazeltonDistance(self.np_sample, (i/100)*self.aaa6[j][0] + ((100-i)/100)*self.aaa6[j][1])
                    
                    self.aa.append(ksks)
#            print(self.aa)
#            print(len(self.aa))
            
            self.max_aa = np.min(np.array(self.aa)[~np.isnan(self.aa)])
            self.max_aa_index=np.argmin(np.array(self.aa)[~np.isnan(self.aa)])
...
#            print(self.aaa[0])
            for j in range (len(self.aaa3)):
                for i in range(101):
                    Ratio_A.append(i/100)
                    ksks=wd.calculate_wasserstein_2(self.np_sample, (i/100)*self.aaa3[j][0] + ((100-i)/100)*self.aaa3[j][1])[0]
                    
                    self.aa.append(ksks)
                    
                    ssss=wd.calculate_wasserstein_2(self.np_sample, (i/100)*self.aaa3[j][0] + ((100-i)/100)*self.aaa3[j][1])[1]
                    self.slope.append(ssss)
                    
                    kkkk=wd.calculate_wasserstein_2(self.np_sample, (i/100)*self.aaa3[j][0] + ((100-i)/100)*self.aaa3[j][1])[2]
...
#                    ax.text(*label_pos, f"{fraction:.1f}", fontsize=8, ha='center', va='center')
            ...
            cbar.set_label("Wasserstein-2", rotation=0, labelpad=5)  # 标签水平放置，更易读
